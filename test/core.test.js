...
test("вибір людини сильніший за систему, а без вибору слухаємо систему", () => {
  assert.equal(startTheme("light", true), "light", "обране світле лишається світлим у темній системі");
  assert.equal(startTheme("dark", false), "dark");
  assert.equal(startTheme(null, true), "dark");
  assert.equal(startTheme(null, false), "light");
  assert.equal(startTheme("сміття", true), "dark", "непридатне значення зі сховища ігноруємо");
});
